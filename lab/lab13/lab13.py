...
def scientific_name(name):
    ...
    False
    """
    return bool(re.search(r"^[A-Z](\.|[a-z]+) [a-z]+$", name))  # TODO maybe a wrong answer
...
def calculator_ops(calc_str):
    ...
    False
    """
    return bool(re.search(r"\([\+\-\*\/] \d+ \d+\)", calc_str))  # TODO maybe a wrong answer
...
def roman_numerals(text):
    ...
    False
    """
    return bool(re.search(r"^M{0,3}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})$", text))
